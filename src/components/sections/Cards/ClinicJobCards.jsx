...
export default function JobList() {
    ...
    useEffect(() => {
        ...
        if (userClinic) {
            ...
            const getJobsByClinicId = async () => {
                const res = await axios.get(`http://localhost:4000/jobs/search/clinic/${userClinic._id}`);
    ...
    const handleChanges = async (event, postid) => {
        ...
        try {
            await axios.delete(`http://localhost:4000/jobs/delete/${postid}`);
    ...
    const addNewJob = (event) => {
        ...
        axios.post('http://localhost:4000/jobs/add', toSend, {
            headers: {
    ...
    const handleSubmit = (event, postid) => {
        ...
        try {
            fetch(`http://localhost:4000/jobs/update/${postid}`, {
                method: "POST",
